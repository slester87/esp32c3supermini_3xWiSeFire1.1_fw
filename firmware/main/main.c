...
static void set_pixels(uint8_t status_r, uint8_t status_g, uint8_t status_b,
                       uint8_t sol_r, uint8_t sol_g, uint8_t sol_b)
{
    if (!strip) {
        return;
    }
...
static void update_status_led_locked(void)
{
    switch (runtime.state) {
        case STATE_BOOT:
            set_pixels(0, 0, 32, 0, 0, 0); // dim blue
            break;
        case STATE_READY:
            set_pixels(0, 64, 0, 0, 0, 0); // green
            break;
        case STATE_FIRING:
            set_pixels(64, 16, 0, 0, 255, 0); // orange + solenoid green
            break;
        case STATE_ERROR:
        default:
            set_pixels(64, 0, 0, 0, 0, 0); // red
            break;
    }
}
...
static void send_state_async(void)
{
    ...
    int len = snprintf(payload, sizeof(payload),
                       "{\"ready\":%s,\"firing\":%s,\"error\":%s,\"elapsed_ms\":%u,\"last_hold_ms\":%u}",
                       ready ? "true" : "false",
                       firing ? "true" : "false",
                       error ? "true" : "false",
                       elapsed,
...
static void handle_press_down(void)
{
    ...
    if (runtime.state == STATE_ERROR || runtime.press_active || runtime.press_ignore_until_release) {
        xSemaphoreGive(state_lock);
        return;
    }
...
static bool wifi_load_credentials(char *ssid_out, size_t ssid_len, char *pass_out, size_t pass_len)
{
    nvs_handle_t nvs;
...
static esp_err_t wifi_post_handler(httpd_req_t *req)
{
    ...
    httpd_resp_sendstr(req, "<html><body><h2>Saved. Reconnecting...</h2><a href=\"/\">Back</a></body></html>");
    return ESP_OK;
}
